...
import {
  CheckCircle2, Circle, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, ArrowLeft,
...
export default function EventWizard({ event }: Props) {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      {/* Sidebar checklist — desktop uniquement */}
      <aside className="hidden lg:flex w-56 flex-shrink-0 flex-col border-r" style={{ background: '#FDFCF9', borderColor: 'rgba(26,15,8,0.07)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {/* Barre mobile : retour + nom + progression + liste d'étapes repliable */}
        <div className="lg:hidden border-b bg-white flex-shrink-0" style={{ borderColor: 'rgba(26,15,8,0.06)' }}>
          <div className="flex items-center justify-between px-4 pt-3">
            <button onClick={() => router.push('/admin/events')}
              className="flex items-center gap-1.5 text-[11px] text-[#9B7A56] hover:text-[#1A0F08] transition-colors">
              <ArrowLeft size={11} />
              Tous les événements
            </button>
          </div>
          ...
          >
            <div className="text-left min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <div className="w-24 h-1 rounded-full bg-[rgba(26,15,8,0.08)]">
                  <div className="h-full rounded-full" style={{ width: `${progress}%`, background: '#B85C28' }} />
                </div>
                <span className="text-[10px] text-[#9B7A56]">{currentIndex + 1}/{STEPS.length} · {currentStep?.label}</span>
              </div>
            </div>
            {mobileStepsOpen
              ? <ChevronUp size={16} className="text-[#9B7A56] flex-shrink-0 ml-2" />
              : <ChevronDown size={16} className="text-[#9B7A56] flex-shrink-0 ml-2" />
            }
          </button>

          {mobileStepsOpen && (
            <nav className="px-2 pb-3 border-t max-h-[60vh] overflow-y-auto" style={{ borderColor: 'rgba(26,15,8,0.06)' }}>
              <div className="pt-2">{renderStepList()}</div>
            </nav>
          )}
        </div>

        {/* Header (index/étape + navigation précédent/suivant) */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b bg-white flex-shrink-0" style={{ borderColor: 'rgba(26,15,8,0.06)' }}>
          ...
          <h2 className="lg:hidden text-[14px] font-semibold text-[#1A0F08]" style={{ fontFamily: 'Playfair Display, serif' }}>
            {currentStep?.label}
          </h2>
