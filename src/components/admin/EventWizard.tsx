...
import {
  CheckCircle2, Circle, ChevronLeft, ChevronRight, Eye, ArrowLeft,
...
const STEPS: BuilderStepKey[] = ['general', 'formula', 'guests', 'seating', 'design', 'sections', 'gallery', 'qrcode', 'publish'];
...
export default function EventWizard({ event }: Props) {
  ...
  function markComplete() {
    completeStep(event.id, currentKey);
  }
    ...
    <div className="flex h-full">
      ...
      >
        ...
        <div className="px-4 pt-4 pb-3 border-b" style={{ borderColor: 'rgba(26,15,8,0.07)' }}>
          ...
          <p className="text-[10px] text-[#9B7A56] mt-0.5">{completedCount}/{STEPS.length} étapes complétées</p>
          {/* Progress bar */}
        ...
        {/* Preview button */}
        <div className="px-3 py-3 border-t" style={{ borderColor: 'rgba(26,15,8,0.07)' }}>
          <button
            onClick={() => router.push(`/admin/events/${event.id}/preview`)}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-[12px] font-medium border transition-colors hover:bg-[rgba(26,15,8,0.03)]"
