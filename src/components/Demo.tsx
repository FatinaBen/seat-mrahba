...
function AccueilScreen() {
  return (
    <div className="flex flex-col items-center text-center px-4 pt-4 pb-2">
      <p className="text-[9px] font-medium tracking-[0.35em] uppercase text-[#8A7235] mb-2">Seat & Mrahba</p>
...
function TableScreen() {
  ...
  return (
    <div className="px-3 py-4 flex flex-col">
      {!searched ? (
        <>
          <p className="text-[10px] font-medium tracking-[0.3em] uppercase text-[#8A7235] mb-3 text-center">Trouver ma table</p>
          <div className="rounded-xl p-3 mb-3" style={{ background: '#F8F4EF', border: '1px solid rgba(232,196,154,0.4)' }}>
            <p className="text-[9px] text-[#9B7A56] mb-2">Renseignez votre nom et prénom</p>
            <div className="rounded-lg px-3 py-2 mb-2 bg-white" style={{ border: '1px solid rgba(232,196,154,0.6)' }}>
              <span className="text-[11px] text-[#9B7A56]">Votre nom complet</span>
            </div>
            <button onClick={() => setSearched(true)} className="w-full py-2 rounded-lg text-[11px] font-medium text-white" style={{ background: '#B85C28' }}>
              Rechercher
            </button>
          </div>
          <p className="text-[9px] text-[#9B7A56] mb-2 text-center">Plan de la salle</p>
          <div className="rounded-xl p-3" style={{ background: '#F8F4EF', border: '1px solid rgba(232,196,154,0.3)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          <p className="text-[10px] font-medium tracking-[0.3em] uppercase text-[#8A7235] mb-3 text-center">Votre place</p>
          <div className="rounded-xl p-4 mb-3 text-center"
            ...
            <p className="text-[22px] text-white font-medium leading-none mb-1" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>
          ...
          <button onClick={() => setSearched(false)} className="mt-3 w-full text-center text-[10px] text-[#9B7A56]">
            ← Nouvelle recherche
          </button>
        </motion.div>
      )}
...
function MenuScreen() {
  return (
    <div className="px-4 py-4">
      <p className="text-[9px] font-medium tracking-[0.35em] uppercase text-[#8A7235] mb-4 text-center">Menu du soir</p>
      {[
        { label: 'Entrée', color: '#8A7235', items: ['Burrata crémeuse & tomates confites', "Huile d'olive et basilic frais"] },
        { label: 'Plat', color: '#B85C28', items: ['Filet de bœuf en croûte', 'Gratin dauphinois & légumes de saison'] },
        { label: 'Dessert', color: '#CF9068', items: ['Pièce montée artisanale', 'Macarons & café'] },
      ].map((section, i) => (
        <div key={i} className="mb-4 last:mb-0">
          <div className="flex items-center gap-2 mb-2">
            <div className="h-px flex-1" style={{ background: `${section.color}40` }} />
            <p className="text-[9px] font-medium tracking-[0.3em] uppercase" style={{ color: section.color }}>{section.label}</p>
            <div className="h-px flex-1" style={{ background: `${section.color}40` }} />
          </div>
          {section.items.map((item, j) => (
            <p key={j} className="text-[11px] text-[#5A3C1E] text-center leading-relaxed mb-0.5">{item}</p>
          ))}
        </div>
      ))}
...
function GalerieScreen() {
  const colors = ['#D4B896','#B85C28','#E8C49A','#8A7235','#CF9068','#9B7A56','#E8C49A','#B85C28','#8A7235','#CF9068','#D4B896','#9B7A56'];
  return (
    <div className="px-3 py-4">
      <p className="text-[9px] font-medium tracking-[0.35em] uppercase text-[#8A7235] mb-1 text-center">Galerie</p>
  ...
  return (
    <section id="demo" className="py-28 md:py-36 relative" style={{ background: '#F8F4EF' }}>
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 lg:px-12">
        ...
        >
          <p className="text-[11px] font-medium tracking-[0.35em] uppercase text-[#8A7235] mb-6">Démonstration</p>
          <h2
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16 justify-center">
          ...
          >
            <div className="space-y-3">
              {tabs.map((tab) => (
                ...
                >
                  <p className="font-medium text-[14px] mb-0.5" style={{ color: activeTab === tab.id ? 'white' : '#1A0F08' }}>
                    {tab.label}
                  </p>
                  <p className="text-[12px]" style={{ color: activeTab === tab.id ? 'rgba(255,255,255,0.65)' : '#9B7A56' }}>
                    {descriptions[tab.id]}
                  </p>
                </button>
              ))}
          ...
          {/* Phone mockup — palette crème, pas de noir */}
          <motion.div
              ...
              >
                ...
                <div className="w-full rounded-[2.6rem] overflow-hidden" style={{ background: '#FDFAF7' }}>
                  ...
                  <div className="px-5 pt-3 pb-4 text-center"
                    ...
                    <p className="text-[8px] font-medium tracking-[0.35em] uppercase text-[#8A7235] mb-1">Seat & Mrahba</p>
